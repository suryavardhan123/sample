...
function service (request)
{
	...
	try
	{
		...
		if (session.isLoggedIn2())
		{
			...
			if (permissions.tranFind > 0 && permissions.tranCustInvc > 0)
			{
				...
				{
					case 'GET':
						// If the id exist, sends the response of Invoice.get(id), else send (Invoice.list(options) || [])
						Application.sendContent(id ? Invoice.get('invoice', id) : Invoice.list({
							types: 'CustInvc'
						,	status: status
						,	page: page
						}));
